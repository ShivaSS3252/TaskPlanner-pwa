...
export const useTasks = (selectedCategory = null) => {
  ...
  // ── CREATE ─────────────────────────────────────────────────────────────
  const createTask = async (text, category = null, options = {}) => {
    if (!text.trim()) return

    isMutating.current = true

    const isOnline = await checkRealConnectivity()
    const { priority = 'medium', dueDate = null } = options
  ...
  // ── UPDATE META (priority / dueDate) ───────────────────────────────────
  const updateTaskMeta = async (id, updates) => {
    isMutating.current = true
    ...
    const updated = { ...existing, ...updates, syncStatus: isOnline ? 'synced' : 'pending' }
    ...
    if (isOnline) {
      try {
        ...
        console.log('Task meta synced ✅')
      } catch (err) {
        console.error('Meta update API failed, queuing:', err)
        await addPendingOp({ entity: 'task', type: 'update', payload: { id, updates } })
      }
    } else {
      await addPendingOp({ entity: 'task', type: 'update', payload: { id, updates } })
    }
